...
interface Props {
  ...
  mintFree(mintAmount: number): Promise<void>;
...
export default class MintWidget extends React.Component<Props, State> {
  ...
  private incrementMintAmount (): void {
    this.setState({
      mintAmount: this.props.totalSupply < this.props.maxFree
        ? Math.min(this.props.maxFreeMintAmountPerTx, this.state.mintAmount + 1)
        : Math.min(this.props.maxMintAmountPerTx, this.state.mintAmount + 1)
    })
  }
  ...
  private async mint (): Promise<void> {
    if (!this.props.isPaused) {
      this.props.totalSupply < this.props.maxFree
        ? await this.props.mintFree(this.state.mintAmount)
